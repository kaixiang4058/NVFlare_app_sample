...
class MSCSUniModel(MRCPSModel):
    def training_step(self, batch, batch_idx):
        ...
        if "unlabel" in batch:
            ...
            y_pred_un_1, y_pred_un_1_fp = self.branch1(image_un, lrimage_un, need_fp=True)
            y_pred_un_2, y_pred_un_2_fp = self.branch2(image_un, lrimage_un, need_fp=True)
            with torch.no_grad():
            ...
            # y_pred_un_1_fp = self.branch1(image_un, lrimage_un, need_fp=True)
            # y_pred_un_2_fp = self.branch2(image_un, lrimage_un, need_fp=True)

            total_pred_1 = torch.cat((y_pred_un_1_fp, mix_pred_1))
            total_pred_2 = torch.cat((y_pred_un_2_fp, mix_pred_2))
