...
from models.psmt.Model.SegFormer.encoder_decoder import *
from models.psmt.Model.SegFormer.segformer import SegFormer
...
class EntireModel(BaseModel):
    def __init__(self, num_classes, data_shape=(512,512), sup_loss=torch.nn.CrossEntropyLoss, cons_w_unsup=None):
        ...
        self.encoder1 = SegFormer(encoder_name=encoder_name, classes=num_classes)
        self.decoder1 = DecoderNetwork(num_classes=num_classes, data_shape=data_shape)
        self.encoder2 = SegFormer(encoder_name=encoder_name, classes=num_classes)
        self.decoder2 = DecoderNetwork(num_classes=num_classes, data_shape=data_shape)
        self.encoder_s = SegFormer(encoder_name=encoder_name, classes=num_classes)
