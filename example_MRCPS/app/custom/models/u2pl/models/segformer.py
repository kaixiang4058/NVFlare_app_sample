...
class SegFormer(nn.Module):
    ...
    def forward(self, inputs):
        feats = self.encoder(inputs).hidden_states
...
class SegformerDecodeHead(SegformerPreTrainedModel):
    def __init__(self, config):
        ...
        self.representer = nn.Sequential(
            nn.Conv2d(config.decoder_hidden_size, 256, kernel_size=3, stride=1, padding=1, bias=True),
            nn.BatchNorm2d(256),
        ...
        self.classifier = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Dropout2d(0.1),
            nn.Conv2d(256, config.num_labels, kernel_size=1, stride=1, padding=0, bias=True),
        )
    ...
    def forward(self, encoder_hidden_states):
        ...
        pred = self.classifier(hidden_states)
        rep = self.representer(hidden_states)
