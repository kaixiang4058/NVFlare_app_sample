    ...
    def training_step(self, batch, batch_idx):
        ...
        if "unlabel" in batch:
            ...
            with torch.no_grad():
                ...
                strong_parameters = {}
                strong_parameters["flip"] = random.randint(1, 7)
            ...
            aug_loss_1 = self.criterion(mix_pred_1, mix_un_mask_1) * self.consistencyratio
            aug_loss_2 = self.criterion(mix_pred_2, mix_un_mask_2) * self.consistencyratio

            self.log(f"train 1 cps loss", cps_loss_1)
            self.log(f"train 2 cps loss", cps_loss_2)

            self.log(f"train 1 aug loss", aug_loss_1)
            self.log(f"train 2 aug loss", aug_loss_2)

            totalloss += (cps_loss_1 + cps_loss_2 + aug_loss_1 + aug_loss_2)
