...
class HisPathDataset(Dataset):
    def __init__(self, datalist: str, pklpath: str, stage: str, data_ratio: dict, \
        ...
        self.temp_wsi_name = None
        self.temp_wsiM_name = None
        self.temp_wsi = None
        self.temp_wsi_lr = None
        self.temp_wsi_mask = None
        warnings.filterwarnings('ignore', message="Loading image.*slow-path")
   ...
   #-- read selection of file from json (with stage, label/unlabel) --
    ...
    def __getitem__(self, idx):
        ...
        if self.temp_wsi_name != tifpath:
            self.temp_wsi_name = tifpath
            
            slide = CuImage(tifpath)
            slide2 = CuImage(tifpath)
            self.temp_wsi = slide
            self.temp_wsi_lr = slide2
            
            # memory_capacity = preferred_memory_capacity(self.temp_wsi, patch_size=self.patchsize)
            memory_capacity = preferred_memory_capacity(self.temp_wsi)
            self.cache = CuImage.cache('per_process', memory_capacity=memory_capacity)
            # cache = CuImage.cache('NoCache', memory_capacity=memory_capacity)
        ...
        region = slide.read_region(location=(x, y), size=(self.patchsize, self.patchsize), level=self.tifpage)
        image = np.asarray(region)
        if image.shape[2] == 4:
            image = image[:, :, 0:3]

        print('= Cache Info =')
        print(f'       type: {self.cache.type}({int(self.cache.type)})')
        print(f'memory_size: {self.cache.memory_size}/{self.cache.memory_capacity}')
        print(f'       size: {self.cache.size}/{self.cache.capacity}')
        print(self.cache.hit_count)
        ...
        # regionlr = slide.read_region(location=(x//self.lrratio-self.lrshift, y//self.lrratio-self.lrshift), size=(self.patchsize, self.patchsize), level=self.tifpage+self.lrdiff)
        regionlr = slide.read_region(location=(x, y), size=(self.patchsize, self.patchsize), level=self.tifpage)
        lrimage = np.asarray(regionlr)
        if lrimage.shape[2] == 4:
            lrimage = lrimage[:, :, 0:3]

        print('= Cache Info Multi =')
        print(f'       type: {self.cache.type}({int(self.cache.type)})')
        print(f'memory_size: {self.cache.memory_size}/{self.cache.memory_capacity}')
        print(f'       size: {self.cache.size}/{self.cache.capacity}')
        print(self.cache.hit_count)
        ...
        if self.islabel:
            ...
            if self.temp_wsiM_name != maskpath:
                slide_mask = self.read_WSI(maskpath, 'tif', level=self.tifpage)
                # slide_mask = CuImage(maskpath)
                self.temp_wsi_mask = slide_mask
            else:
                slide_mask = self.temp_wsiM_name
...
#             region_m = slide_mask.read_region(location=(x, y), size=(self.patchsize, self.patchsize), level=self.tifpage)
            
#             mask = np.asarray(region_m)
#             mask = mask[:, :, 0]
